...
export default function LessonView() {
  ...
  // Render content with glossary tooltips
  const renderContentWithTooltips = useMemo(() => {
    if (!lesson?.content || !lesson.glossaryTerms) return lesson?.content || "";
    
    let content = lesson.content;
    const terms = lesson.glossaryTerms;
    
    // Sort terms by length (longest first) to avoid partial replacements
    const sortedTerms = [...terms].sort((a, b) => b.term.length - a.term.length);
    
    return content;
  }, [lesson?.content, lesson?.glossaryTerms]);
                            ...
                            {lesson.glossaryTerms.map((term) => (
                              <Tooltip key={term.id}>
                                <TooltipTrigger asChild>
                                  <div className="p-3 rounded-lg bg-background cursor-help">
                          ...
                          return (
                            <Card key={index} className={feedback ? (feedback.score >= 70 ? "border-green-500" : "border-red-500") : ""}>
                              ...
                              <CardContent>
                                ...
                                {feedback && (
                                  <div className={`mt-4 p-3 rounded-lg ${feedback.score >= 70 ? "bg-green-50 text-green-800" : "bg-red-50 text-red-800"}`}>
